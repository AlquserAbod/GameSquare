...
import { checkWinPattern, determineMatchWinner, resetBoard, toggleCurrentPlayer } from '../../Games/tictactoe/GameLogic.js';
...
export default function handleGameEvents(io, socket) {
    ...
    socket.on("start_game", (room) => {
        ...
        const game = {
            roomId: room.id,
            player1: JSON.parse(room.members.find((member) => JSON.parse(member).role == 'player1')),
            player2: JSON.parse(room.members.find((member) => JSON.parse(member).role == 'player2')),
            rounds: [],
    ...
    socket.on("make_move", async ({room, position}) => {
        try {
            ...
            const player =  JSON.parse(room.members.find(member => JSON.parse(member).id == socket.id));             
            ...
            if(winPattern != undefined) {
                const roundWinner = (player.role === 'player1') ? game.player1 : game.player2;
